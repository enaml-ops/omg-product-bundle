	...
	Context("when calling GetProduct w/ valid flags and matching cloud config", func() {
		...
		BeforeEach(func() {
			...
			dmBytes, err := plgn.GetProduct([]string{
				...
				"--notification-recipient-email", "no-reply@example.com",
				"--proxy-ip", "10.0.1.2",
				"--monitoring-ip", "10.0.1.3",
				"--broker-ip", "10.0.1.4",
				"--admin-password", "admin",
				"--notifications-client-secret", "secret",
				"--uaa-admin-secret", "secret",
				"--nats-pass", "pass",
				"--nats-machine-ip", "10.0.1.5",
				"--mysql-admin-password", "foo",
				"--seeded-db-password", "password",
				"--galera-healthcheck-username", "user",
				"--galera-healthcheck-password", "password",
				"--galera-healthcheck-db-password", "password",
				"--cluster-health-password", "password",
				"--backup-endpoint-user", "backup",
				"--backup-endpoint-password", "pass",
				"--broker-quota-enforcer-password", "pass",
				"--proxy-api-password", "pass",
				"--broker-auth-username", "broker",
				"--broker-auth-password", "password",
				"--broker-cookie-secret", "cookies",
				"--service-secret", "shhhhh",
			}, cloudConfigBytes, nil)
