...
var _ = Describe("Smoke test errand", func() {
	Context("when initialized WITH a complete set of arguments", func() {
		...
		It("then it should then have smoke-tests job", func() {
			...
			Ω(props.Cf.AppDomains).Should(Equal("apps.test.com"))
			Ω(props.Cf.ApiUrl).Should(Equal("https://api.sys.test.com"))
			Ω(props.Cf.SmokeTests.Org).Should(Equal("CF_SMOKE_TEST_ORG"))
			Ω(props.Cf.AdminUsername).Should(Equal("smoke_tests"))
			Ω(props.Cf.AdminPassword).Should(Equal("password"))
			Ω(props.Cf.SmokeTests.UseExistingOrg).Should(BeFalse())
